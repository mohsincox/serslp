...
const pointTableGetAll = (req, res) => {
  helper
    .checkPermission(req.user.role_id, "point_table_view")
    .then((rolePerm) => {
      PointTable.findAll({
        include: [
          {
...
const footballPointTableAdd = async (req, res) => {
  ...
  try {
    ...
    const totalPoint = await helper.calculateFootballPlayerTotalPoint(
      footballPoints,
      currentPlayerSpcification
    );
    const userTeamPointUpdate = await TeamDetail.increment(
...
const footballPointTableUpdate = async (req, res) => {
  ...
  try {
    ...
    try {
      let pointTable = await PointTable.findByPk(req.params.id, {
        include: [{ model: Player }, { model: Match }],
      });
      let fixTeamDetailsPoint = await helper.previousTeamDetailDecrement(
        pointTable
      );
      const totalPoint = await helper.calculateFootballPlayerTotalPoint(
        footballPoints,
